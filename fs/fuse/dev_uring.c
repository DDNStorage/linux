...
static int fuse_uring_copy_to_ring(struct fuse_ring_ent *ring_ent,
				   struct fuse_req *req)
{
	struct fuse_ring_queue *queue = ring_ent->queue;
	struct fuse_ring *ring = queue->ring;
	...
	if (ring_ent->header_pages) {
		headers = kmap_local_page(
			ring_ent->header_pages[FUSE_RING_HEADER_PG]);

		memcpy(&headers->in_out, &req->in.h, sizeof(req->in.h));

		err = fuse_uring_args_to_ring_pages(ring, req, ring_ent,
						    headers);
		kunmap_local(headers);
	} else {
		/* copy the request */
		err = fuse_uring_args_to_ring(ring, req, ring_ent);
		if (unlikely(err)) {
			pr_info_ratelimited("Copy to ring failed: %d\n", err);
			return err;
		}
		err = copy_to_user(&ring_ent->headers->in_out, &req->in.h,
				   sizeof(req->in.h));
		if (err)
			err = -EFAULT;
	}
...
static int fuse_uring_prepare_send(struct fuse_ring_ent *ring_ent)
{
	struct fuse_req *req = ring_ent->fuse_req;
	int err;

	err = fuse_uring_copy_to_ring(ring_ent, req);
	if (!err)
